...
export default function JogoDoBichoScreen() {
  ...
  function gerarAnimalAleatorio() {
    const indexAleatorio = Math.floor(Math.random() * animais.length)
    setAnimalSelecionado(animais[indexAleatorio])
  }

  return (
    <View style={styles.container}>
      <Text style={styles.titulo}>Jogo do Bicho</Text>

      {animalSelecionado && (
        <View style={styles.card}>
          <Image
            source={{ uri: animalSelecionado.imagem }}
            style={styles.imagem}
            resizeMode="cover"
          />
          <Text style={styles.nome}>{animalSelecionado.nome}</Text>
        </View>
      )}

      <Button title="Gerar Animal Aleatório" onPress={gerarAnimalAleatorio} />
    </View>
  )
}
